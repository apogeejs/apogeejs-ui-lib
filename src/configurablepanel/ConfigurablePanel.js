...
export default class ConfigurablePanel {
    ...
    */
     ...
     * It also returns a flag indicating if the function body is needed to evaluate the value - hasExpression
     * return value: {hasExpression, functionBody}
    */
    static createResultFunctionBody(formValue,formLayout) {
        let outputStruct = {
            hasExpression: false,
            lineArray: []
        }
        ...
        formLayout.forEach( childLayout => ConfigurablePanel.processPanelLayout(childLayout,formValue,outputStruct))
    ...
    static processPanelLayout(childLayout,panelValue,outputStruct) {
        if(childLayout.key) {
            //this is a ConfigurableElement with a value
            let childValue = panelValue[childLayout.key]
            if(childValue !== undefined) {
                let {hasExpression, valueCodeText} = ConfigurablePanel.createElementValueCodeText(childValue,childLayout,panelValue)
                let lineCodeText = `${childLayout.key}:${valueCodeText}`
                outputStruct.lineArray.push(lineCodeText)
            ...
            layoutLayout.forEach( childLayout => ConfigurablePanel.processPanelLayout(childLayout,panelValue,outputStruct))
        }
    }
     ...
     * overridden for elements with alternate code translations. */
     static createElementValueCodeText(value,layout,containerValue) {
        ...
        if(elementClass.createValueCodeText) return elementClass.createValueCodeText(value,layout,containerValue)
